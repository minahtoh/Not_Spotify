...
import com.example.notspotify.other.Event
...
class MusicServiceConnection(
    ...
    val currPlayingSong :LiveData<MediaMetadataCompat?> = _currentPlayingSong
       ...
       override fun onSessionEvent(event: String?, extras: Bundle?) {
           ...
           when(event){
               NETWORK_ERROR -> _networkError.postValue(Event
                   (
                   Resource.error("Couldn't connect to the server, Please check internet connection",null)
               )
               )
           }
       }
